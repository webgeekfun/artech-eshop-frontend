        ...
        if (!this.$store.state.isLogin) {
          ...
          if (this.$store.state.nickNameInfo) {
            ...
            if (newVal.length > oldVal.length) {
              ...
              api = '/wishlist/add?skuId=';
            } else {
              diff = oldVal.filter(item => !newSet.has(item));
              api = '/wishlist/remove?skuId=';
            }
            ...
            if (newVal.length === oldVal.length) {
              newVal.forEach((element, index) => {
                if (element.count != oldVal[index].count) {
                  api = '/cart/update?skuId=' + element.skuId + '&count=' + element.count;
                }
              });
            } else if (newVal.length < oldVal.length) {
              // 删除产品
              diff = oldVal.filter(
                item => this.$findElem(newVal, 'skuId', item.skuId) == -1
              );
              api = '/cart/update?skuId=' + diff[0].skuId + '&count=0';
            } else {
              //新增产品
              diff = newVal.filter(
                item => this.$findElem(oldVal, 'skuId', item.skuId) == -1
              );
              api = '/cart/add?skuId=' + diff[0].skuId + '&count=' + diff[0].count;
            }
