...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    toggleWishlist(state, productId) {
      const tempProductId = Number(productId);
      if (state.wishlist.indexOf(tempProductId) === -1) {
        state.wishlist.push(tempProductId);
      } else {
        if (location.href.indexOf('cart') == -1) {
          state.wishlist.splice(state.wishlist.indexOf(tempProductId), 1);
        }
      }
    },
    ...
    toggleCart(state, item) {
      const idx = Vue.prototype.$findElem(state.cart, 'productId', item.productId);
      if (idx === -1) {
        ...
        if (item.count === 0) {
          ...
          // 从购物车删除之后，需要添加到心愿单中
          this.commit('toggleWishlist', item.productId);
        } else {
          // 修改产品数量
          if (location.href.indexOf('cart') != -1) {
    ...
    initCartFromBoth(context) {
      ajax.get('/cart/list', '', 2, 'mustAuth', response => {
        if (response.status >= 200 && response.status < 300) {
          ...
          diff.forEach(item => {
            ajax.post(
              '/cart/add?productId=' + item.productId + '&count=' + item.count,
              '',
              2,
              'mustAuth',
              response => {
    ...
    initWishlistFromBoth(context) {
      ajax.get('/wishlist/list', '', 2, 'mustAuth', response => {
        if (response.status >= 200 && response.status < 300) {
          ...
          diff.forEach(item => {
            ajax.post('/wishlist/add?productId=' + item, '', 2, 'mustAuth', response => {
              if (response.status >= 200 && response.status < 300) {
                console.log(response.message);
              }
            });
